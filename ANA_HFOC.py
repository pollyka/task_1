...
parser.add_argument("-f", "--file", nargs = "+", help="The root file for data")
parser.add_argument("-l", "--label", nargs = "+", help="The label of the output file")
parser.add_argument("-t", "--threshold", nargs = "+", help="The ADC threshold")
...
for f in range(len(filename)):
    tfile = ROOT.TFile(filename[f])
    ttree = tfile.Get("HFtree")
    fout = ROOT.TFile("HFOC_threshold"+thresholds[f]+"_new_Phase2_"+labels[f]+".root", "RECREATE")
    ...
    for ievt in range(nevts):
        ...
        if len(PU_list[pu])==0:
            for iCh in range(nCh):
                ...
                if ttree.ADC_31[iCh]<=int(thresholds[f]):
            ...
            for iCh2 in range(nCh2):
                #print "ADC", ttree.ADC_31[iCh]
                if ttree.ADC_32[iCh2]<=int(thresholds[f]):
                    PU_list[pu].append([1,1])
                else:
                    PU_list[pu].append([0,1])
            ...
            for iCh in range(nCh):
                calc_zerofrac(PU_list[pu][iCh], bool((ttree.ADC_31[iCh]>int(thresholds[f]))))
            for iCh in range(nCh2):
                calc_zerofrac(PU_list[pu][iCh+nCh], bool((ttree.ADC_32[iCh]>int(thresholds[f]))))
        #print PU_list[pu]
        #print PU_list[pu]
    print(PU_list)
